...
a novel way to estimate the a posteriori distribution of Bayesian
remote sensing retrievals.

 A synthetic retrieval scenario is presented and
...
cloud top pressure (CTP) from observations by the moderate resolution
imaging spectroradiometer (MODIS). It is shown that QRNNs are not only
capable of achieving similar accuracy as state-of-the-art neural network
retrievals, but also provide statistically consistent uncertainty estimates
 for non-Gaussian retrieval errors. Furthermore, the implementation of QRNNs
requires only minor changes to the training and evaluation workflow of standard
neural networks, thus retaining their computational efficiency and flexibility.
...
Together with this article, an implementation of QRNNs in Python is released
through a public repository to make the method available to the
...
\texit{during-training validation set} from the original article. For the
...
calculations presented in Section~\ref{sec:synthetic} and \ref{sec:ctp} are
